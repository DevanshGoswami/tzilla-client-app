        ...
        return (
            ...
            >
                <View style={{ paddingHorizontal: 12 }}>
                    <SvgText fill="#6B7280">Log a couple of weights to see your trend.</SvgText>
                </View>
            </View>
        );
    }
    ...
    return (
        <View style={{ height, borderRadius: 12, overflow: "hidden", backgroundColor: "#fff" }}>
            <Svg width="100%" height="100%" viewBox={`0 0 ${width} ${height}`}>
                ...
                {/* Axes */}
                <Line x1={padding} x2={padding + innerW} y1={padding + innerH} y2={padding + innerH} stroke="#D1D5DB" />
                <Line x1={padding} x2={padding} y1={padding} y2={padding + innerH} stroke="#D1D5DB" />
                ...
                <G>
                    {yTickVals.map((yv, i) => (
                        <SvgText key={`yt-${i}`} x={padding - 8} y={sy(yv) + 3} fontSize="10" fill="#6B7280" textAnchor="end">
                            {`${Math.round(yv)} kg`}
                        </SvgText>
                    ))}
                </G>
                ...
                {selPt ? (
                    <G>
                        {/* pointer */}
                        <Line x1={tooltipX} x2={tooltipX} y1={tooltipY} y2={tipRectY + tipH} stroke="#9CA3AF" strokeDasharray="3,3" />
                        {/* box */}
                        <Rect x={tipRectX} y={tipRectY} width={tipW} height={tipH} rx={8} ry={8} fill="#111827" opacity={0.95} />
                        <SvgText
                            x={tipRectX + 8}
                            y={tipRectY + 16}
                            fontSize="11"
